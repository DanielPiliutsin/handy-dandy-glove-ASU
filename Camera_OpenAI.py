...
def capture_image(path: str):
    ...
    subprocess.run(
        ["fswebcam", "-r", RESOLUTION, "-S", "2", "--no-banner", path], check=True
    )
    ...
    try:
        for item in getattr(resp, "output", []):
            for part in getattr(item, "content", []):
                if getattr(part, "type", None) in ("output_text", "text") and getattr(
                    part, "text", None
                ):
...
def main():
    subprocess.run(["espeak", "'Proccessing Image, Please wait!'"])
    try:
